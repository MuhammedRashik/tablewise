...
export default function OtpVerify() {
  ...
  const { joinQueue }    = useQueue(restaurantId);
  ...
  // ── After successful login → handle pending queue join ────────────────
  useEffect(() => {
    if (!isSuccess) return;
    ...
    if (pending) {
      ...
      queueApi
        .getAvailableTables(rid, partySize)
        .then((res) => {
          ...
          if (available.length > 1) {
            ...
            // 0 or 1 table → join directly
            joinQueue({ partySize, notes });
          }
        })
        .catch(() => {
          // If check fails → just join (waitlist or direct)
          joinQueue({ partySize, notes });
        });

    } else {
      // No pending join → go back to the restaurant join page
  ...
  return (
    <div className="screen">
      ...
      {/* REQUIRED by Firebase — invisible reCAPTCHA anchor */}
      {/* Never remove this div — Firebase needs it even in invisible mode */}
